...
Note the split toolchain: the dev server is `react-scripts`, but the production bundle is built by the custom `webpack.config.js`. **Cloudflare Pages runs its own `npm run build`** from the connected Git repo (`alleth/reviewer-app`, `master` branch) — it does *not* serve the committed `webroot/react-frontend/dist/main.js`, so that checked-in artifact is vestigial (harmless to leave, no need to rebuild it per change). What actually ships is whatever Pages builds, using the **`REACT_APP_API_URL`** and **`REACT_APP_GOOGLE_CLIENT_ID`** values set in the Pages project's env vars (injected at build time via `DefinePlugin`; the webpack fallback for `REACT_APP_API_URL` is `http://localhost/reviewer_app`). A **git push to `master`** builds *and* promotes to production; the dashboard "Retry deployment" builds but does **not** promote. Prod: `REACT_APP_API_URL=https://api.career-pass.org` (Railway backend), site served at `https://career-pass.org`.
...
Migrations: topics/questions/choices in `20260529000000_CreateTopicsQuestionsChoices.php`; `auth_codes` + `login_events` in `20260907000000_CreateAuthCodes.php`; `login_attempts` in `20260908000000_CreateLoginAttempts.php`. **`docker-start.sh` does not run migrations**, so on Railway apply new tables/columns by hand:
```sql
