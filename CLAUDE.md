...
- **`users`** — PK: `user_id`. Fields: `fname`/`lname` (max 45), `email` (max 35, unique rule), `user_name` (max 35, unique rule), `user_pass` (`password_hash` digest — **`VARCHAR(255)`**, not 64; null for Google-only accounts), `google_id` (nullable, unique at the DB level via `uniq_google_id`; set by `googleLogin()`), `session_token` (`VARCHAR(64)` nullable — the single-device session guard, see Authentication). Only `google_id` has a DB-level unique constraint; `email`/`user_name` uniqueness is app-level only (`UsersTable` build rules) — pre-existing behavior, not something the migration below changed. **`users` predates migrations** and already exists for real on every DB (local + Railway) — its migration, `20260528000000_CreateUsers.php`, exists **only** so the SQLite test schema (built from scratch, purely from migrations) has something for `users`-FK migrations (`auth_codes`, `passes`, `topic_reviews`) to reference; it must never actually run against a real database. If a fresh real DB ever needs it (a new environment, a rebuilt Railway MySQL service), run `bin/cake migrations mark_migrated --target 20260528000000 --only` instead of letting `migrate` create it, then apply the rest by hand:
  ```sql
  ALTER TABLE users ADD COLUMN session_token VARCHAR(64) NULL;
  ALTER TABLE users MODIFY user_pass VARCHAR(255) NULL;   -- was VARCHAR(64); widen so a future PASSWORD_DEFAULT hash isn't truncated
  ```
...
Migrations: `users` in `20260528000000_CreateUsers.php` (test-schema-only — see the `users` bullet above, **never** run for real); topics/questions/choices in `20260529000000_CreateTopicsQuestionsChoices.php`; `auth_codes` + `login_events` in `20260907000000_CreateAuthCodes.php`; `login_attempts` in `20260908000000_CreateLoginAttempts.php`; `passes` in `20260909000000_CreatePasses.php`; `topic_reviews` in `20260910000000_CreateTopicReviews.php`. **`docker-start.sh` does not run migrations**, so on Railway apply new tables/columns by hand — but **before ever running `bin/cake migrations migrate` there for the first time since this file was added**, run `bin/cake migrations mark_migrated --target 20260528000000 --only` against Railway first, or `migrate` will try to `CREATE TABLE users` against a database where it already exists and fail:
```sql
CREATE TABLE auth_codes (auth_code_id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY, user_id INT NOT NULL, purpose VARCHAR(30) NOT NULL, code_hash VARCHAR(255) NOT NULL, expires DATETIME NOT NULL, attempts INT NOT NULL DEFAULT 0, created DATETIME NULL, INDEX (user_id, purpose), FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE CASCADE);
CREATE TABLE login_events (login_event_id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY, user_id INT NOT NULL, device_hash VARCHAR(64) NOT NULL, created DATETIME NULL, INDEX (user_id, created), FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE CASCADE);
CREATE TABLE login_attempts (login_attempt_id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY, identifier VARCHAR(255) NOT NULL, created DATETIME NULL, INDEX (identifier, created));
CREATE TABLE passes (pass_id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY, user_id INT NOT NULL, reviewer VARCHAR(50) NOT NULL, plan_id VARCHAR(20) NOT NULL, amount DECIMAL(10,2) NOT NULL, currency VARCHAR(3) NOT NULL DEFAULT 'PHP', external_id VARCHAR(64) NOT NULL, xendit_invoice_id VARCHAR(64) NULL, status VARCHAR(20) NOT NULL DEFAULT 'pending', paid_at DATETIME NULL, expires_at DATETIME NULL, created DATETIME NULL, modified DATETIME NULL, UNIQUE KEY (external_id), INDEX (user_id, status, expires_at), FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE CASCADE);
CREATE TABLE topic_reviews (topic_review_id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY, topic_id INT UNSIGNED NOT NULL, author_id INT NULL, content TEXT NOT NULL, created DATETIME NULL, modified DATETIME NULL, UNIQUE KEY (topic_id), FOREIGN KEY (topic_id) REFERENCES topics(topic_id) ON DELETE CASCADE, FOREIGN KEY (author_id) REFERENCES users(user_id) ON DELETE SET NULL);  -- author_id is signed (no UNSIGNED) to match users.user_id
```

**`user_id`/`author_id` FK signedness — fixed.** `users.user_id` is a **signed** `int(11)` (it predates migrations — see the `users` bullet above), but the `auth_codes`/`login_events`/`passes` migrations originally declared their `user_id` FK column `INT UNSIGNED`, which MySQL rejects (`errno 150`, "Foreign key constraint is incorrectly formed") — confirmed locally: `bin/cake migrations migrate` failed outright at `CreateAuthCodes` against a real `users` table, and the local `auth_codes` table that existed before this fix had no FK at all (whoever hand-applied it must have dropped the `FOREIGN KEY` clause to get the `CREATE TABLE` to succeed at all). All three migrations, `topic_reviews.author_id`, and the Railway hand-SQL block above are now **signed** (no `UNSIGNED`) to match `users.user_id`; `bin/cake migrations migrate` now applies cleanly with real FK constraints on every table. **`login_attempts` was never affected** — it has no `users` FK by design (see its bullet above).
